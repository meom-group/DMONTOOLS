function [fig1]=plot_sea_icenoaa(fig,filename,name,figname,print_fig,style);
%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%% READ
X=load(filename) ;
yrs=X(2:end,1) ; mnths=X(1,2:13) ; yr1 = X(2, 1 ) ; X=X(2:end,2:end); 
yr2 = X(end, 1 ) ;
...
ii=0;
                ...
                if print_fig == 1
%               cmd=sprintf('%s  %s%s%s','print','-dpsc sea_icenoaa.',figname,'.ps') ;
%               eval(cmd)
                cmd=sprintf('%s  %s %s%s%s%s%s','print','-djpeg100 ',plotdir,'/','sea_icenoaa.',figname,'.jpg') ;
                eval(cmd)
                end
