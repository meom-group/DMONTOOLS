...
"""tslateq : north latitude-band average temperature and salinity budgets for drakkar runs
"""
#-----------------------------------------------------------------------
    ...
    if fromfiles!=[]:                      # diagnostic mode... 
	  ...
	  sys.exit()
       ...
       if os.path.isfile(filenc):
          ...
          print '>>> tslateq.py : No files found for frequency : ' + argdict['monitor_frequency'] ; exit()
...
def plot(argdict=myargs,figure=None,color='r',tmean=None,smean=None,sshmean=None,levels=None,date=None,tmodel=None,smodel=None,compare=False):
    ...
    titles3Dmean = ['3D 60S-60N T Mean', '3D 60S-60N S Mean', '3D 60S-60N SSH Mean']
...
def save(argdict=myargs,figure=None):
    ...
    figure.savefig(plotdir_confcase + '/' + config + '-' + case + '_' + monit_freq + '_tslateq.png')
