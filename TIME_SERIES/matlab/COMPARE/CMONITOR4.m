...
name         = strcat('green: ',run1,' black: ',run2,' red: ',run3,' magenta: ', run4) ;
figname      = strcat(run1,'_',run2,'_',run3,'_',run4) ;
style1   = 'g.-' ;
style2   = 'k.-' ;
style3   = 'r.-' ;
style4   = 'm.-' ;
...
sea_icemonth   = 0  ; %% timeseries of spring/fall/annual north/south ice volume/area
...
tsmean_lev     = 1  ; %% timeseries of SSHmean, T/Smean and T/S profiles
...
tiret       =findstr(run4,'-') - 1 ; config      = strcat(run4(1:tiret),'/') ;
datadir=strcat(datadirpth,'/',config,run4,'/DATA/') ;
tmean4 = strcat(datadir,run4,'_TMEAN.mtl') ;
TMP=load(tmean4) ; [nb(4),dum] = size ( TMP(2:end,1) ) ; ydeb4=TMP(2,1) ;

nbymin=min(nb)
ydeb=max([ydeb1, ydeb2, ydeb3, ydeb4]) ;
...
for irun = 1:4
  ...
  if irun == 4 ; run=run4 ; style=style4 ; end
  tiret       =findstr(run,'-') - 1 ;
