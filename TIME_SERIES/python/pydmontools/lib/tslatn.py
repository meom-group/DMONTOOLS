...
"""tslatn : north latitude-band average temperature and salinity budgets for drakkar runs
"""
#-----------------------------------------------------------------------
    ...
    if fromfiles!=[]:                      # diagnostic mode... 
	  ...
	  sys.exit()
       ...
       if os.path.isfile(filenc):
          ...
          print '>>> tslatn.py : No files found for frequency : ' + argdict['monitor_frequency'] ; exit()
...
def plot(argdict=myargs,figure=None,color='r',tmean=None,smean=None,sshmean=None,levels=None,date=None,tmodel=None,smodel=None,compare=False):
    ...
    titles3Dmean = ['3D Northward of 60N T Mean', '3D Northward of 60N S Mean', '3D Northward of 60N SSH Mean']
    nb3Dmean=len(list3Dmean)

    for k in range(nb3Dmean) :
            ax1 = figure.add_subplot(3,nb3Dmean,k+1)
            zvar=vars()[list3Dmean[k]]   
            ax1.plot(date, zvar, color + '.-')
            ax1.grid(True)
            ax1.axis([min(_date), max(_date),min(zvar),max(zvar)])
    ...
    zvart= tmodel[-1,:]-tmodel[0,:]
    plt.subplot(3,nb3Dmean,nb3Dmean+1)
    plt.semilogy(zvart,levels,color)
    plt.grid(True)
    plt.axis([min(zvart), max(zvart),max(levels),min(levels)])
    plt.ylabel('Log Depth')
    plt.title('NORTH\n Global change in T')

    zvars= smodel[-1,:]-smodel[0,:]
    plt.subplot(3,nb3Dmean,2*nb3Dmean+1)
    plt.semilogy(zvars,levels,color)
    plt.grid(True)
    plt.axis([min(zvars), max(zvars),max(levels),min(levels)])
    plt.ylabel('Log Depth')
    plt.title('NORTH\n Global change in S')
...
def save(argdict=myargs,figure=None):
    ...
    figure.savefig(plotdir_confcase + '/' + config + '-' + case + '_' + monit_freq + '_tslatn.png')
