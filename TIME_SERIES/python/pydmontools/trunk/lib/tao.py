...
plot_name = 'tao'
fig_size =  [8.,4.] # a editer surement
...
def _readnc(filenc=None,fileobs1=None,fileobs2=None,fileobs3=None,fileobs4=None,fileobs5=None,argdict=myargs):
	...
	for j in range(1,6):
		...
		#outdict['m_Valeur_sous_courant'+str(j)] = rs.readfilenc(filenc,'u_'+coordonnees[j-1]+'_UC') # worked in 2.5
...
##plot profil
def plot1(argdict=myargs, figure=None, color='r', compare=False, **kwargs):
	fig_size=[8. , 4.]
	plt.rcParams['figure.subplot.right']=0.96
	plt.rcParams['figure.subplot.left']=0.08
	plt.rcParams['figure.subplot.top']=0.79
	plt.rcParams['figure.subplot.bottom']=0.12
	plt.rcParams['figure.subplot.wspace']=0.31
	plt.rcParams['legend.numpoints']=2
	plt.rcParams['legend.axespad']=-0.2
	#
...
##plot serie
def plot2(argdict=myargs, figure=None, color='r', compare=False, **kwargs): 
...
def save1(argdict=myargs,figure=None):
    if figure is None:
       figure = plt.gcf()
    config,case = argdict['config'], argdict['case']
    plotdir_confcase = plotdir + '/' + config + '/PLOTS/' + config + '-' + case + '/TIME_SERIES/'
    figure.savefig(plotdir_confcase + '/' + config + '-' + case + '_tao_profil.png')
    return filename

def save2(argdict=myargs,figure=None):
    if figure is None:
       figure = plt.gcf()
    config,case = argdict['config'], argdict['case']
    plotdir_confcase = plotdir + '/' + config + '/PLOTS/' + config + '-' + case + '/TIME_SERIES/'
    figure.savefig(plotdir_confcase + '/' + config + '-' + case + '_tao_serie.png')
    return filename
...
def main(): 
   ...
   #fig1
   fig1 = plot1(argdict=argdict,**values)
   if len(args)==0:
      save1(argdict=argdict,figure=fig1)
   else:
      fig1.savefig('./tao_profil.png')
   fig2 = plot2(argdict=argdict,**values)
   #fig2
   if len(args)==0:
      save2(argdict=argdict,figure=fig2)
   else:
      fig2.savefig('./tao_serie.png')
