...
plt.contourf(year, levels, transpose(tmodel[:,:]-tmodel[0,:]),ncontour)
plt.colorbar()
plt.grid(True)
plt.yscale('log')
...
plt.contourf(year, levels, transpose(smodel[:,:]-smodel[0,:]),ncontour)
plt.colorbar()
plt.grid(True)
plt.yscale('log')
plt.axis([ min(year), max(year), max(levels),min(levels) ])
...
plt.savefig(plotdir_confcase + '/' + config + '-' + case + '_tsmean.png')
