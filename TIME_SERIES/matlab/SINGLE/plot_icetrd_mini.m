function [fig1]=plot_icetrd_mini(fig,filename,name,print_fig);
%%%%%%%%%%%%%%%%%%%%%%%
...
global dataobsdir plotdir
  ...
  % NOAA
  tab=load(strcat('../tmp/','/N_noaa_m09_ice.mtl'));
  ...
  % NOAA
  tab=load(strcat('../tmp/','/S_noaa_m03_ice.mtl'));
  SouthAreaNoaa=tab(:,4) ;
  ...
  plot(year,vn,'k.-') ; grid on ; axis tight ; title('Volume Arctic September')

  % AREA
  subplot(2,3,2) ; hold on
  plot(year,an,'k.-') ; grid on ;  title('Area Arctic September')
 if year(1) > 1900 ;   plot(datnoaa,NorthAreaNoaa,'b.-') ; axis tight ; end

  % EXTENT
  subplot(2,3,3) ; hold on
  plot(year,en,'k.-') ; grid on ;  title('Extent Arctic September')
  ...
  plot(year,vs,'k.-') ; grid on ; axis tight ; title('Volume Antarctic March')

  % AREA
  subplot(2,3,5) ; hold on
  plot(year,as,'k.-') ; grid on ;  title('Area Antarctic March')
  xlabel(config,'fontsize',16)
  if year(1) > 1900 ; plot(datnoaa,SouthAreaNoaa,'b.-') ; axis tight ; end

  % EXTENT
  subplot(2,3,6) ; hold on
  plot(year,es,'k.-') ; grid on ;  title('Extent Antarctic March')
  if year(1) > 1900 ;   plot(datnoaa,SouthExtentNoaa,'b.-') ; axis tight ; end

                if print_fig == 1
%                cmd=sprintf('%s  %s  %s%s ','print','-dpsc', name,'_sea_icetrd.ps') ;
%                eval(cmd)
                cmd=sprintf('%s  %s  %s%s%s%s ','print','-djpeg100', plotdir,'/',name,'_sea_icetrd_mini.jpg') ;
                eval(cmd)
                end
