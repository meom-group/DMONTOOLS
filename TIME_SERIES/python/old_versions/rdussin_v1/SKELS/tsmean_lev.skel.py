...
f1=open(datadir + 'CCOONNFFIIGG-CCAASSEE_TMEAN_lev.mtl','r')
lignes1=[lignes1 for lignes1 in f1.readlines() if lignes1.strip() ] # remove empty lines
f1.close()

f2=open(datadir + 'CCOONNFFIIGG-CCAASSEE_SMEAN_lev.mtl','r')
...
fig_size =  [15.,18.]
datalist1 = ['unused1','unused2','unused3','levels'] # line 2 in TMEAN_lev.mtl and SMEAN_lev.mtl
...
datalist2 = ['year', 'sshmean', 'tmean', 'tmodel'] # following lines in TMEAN_lev.mtl
datalist3 = ['year2', 'sshmean2','smean', 'smodel'] # following lines in SMEAN_lev.mtl
...
for k in range(len(datalistlev1)) :
	vars()[datalistlev1[k]]=[]

for k in range(len(datalistlev2)) :
	vars()[datalistlev2[k]]=[]
...
### 2) 2D PLOTS
# Not very great coded... yet
ncontour=25

plt.subplot(3,nb3Dmean,nb3Dmean+1)
plt.semilogy((tmodel[-1,:]-tlev[:]),levels,'r')
plt.grid(True)
plt.axis([min(tmodel[-1,:]-tlev[:]), max(tmodel[-1,:]-tlev[:]),min(levels),max(levels)])
plt.ylabel('Log Depth')
plt.title('Global change in T')

plt.subplot(3,nb3Dmean,nb3Dmean+2)
plt.contourf(year, log10(levels), transpose(tmodel[:,:]-tlev[:]),ncontour)
plt.colorbar()
plt.grid(True)
...
plt.grid(True)
plt.axis([min(smodel[-1,:]-slev[:]), max(smodel[-1,:]-slev[:]),min(levels),max(levels)])
plt.ylabel('Log Depth')
plt.title('Global change in S')

plt.subplot(3,nb3Dmean,2*nb3Dmean+2)
plt.contourf(year, log10(levels), transpose(smodel[:,:]-slev[:]),ncontour)
plt.colorbar()
plt.grid(True)
...
plt.savefig(plotdir + 'CCOONNFFIIGG-CCAASSEE_tsmean_lev.png')
#plt.savefig(plotdir + 'CCOONNFFIIGG-CCAASSEE_tsmean_lev.jpg',format='jpg')
