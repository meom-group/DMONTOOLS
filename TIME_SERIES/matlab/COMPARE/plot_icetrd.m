function [fig1]=plot_icetrd(fig,config,filename,name,figname,print_fig,style,maxyear);
%%%%%%%%%%%%%%%%%%%%%%%
...
if maxyear ~= 0 ; if yr2 > maxyear ; yr2 = maxyear ; end ; end
  ...
  % NOAA
  tab=load('../tmp/N_noaa_m03_ice.mtl');
  ...
  % NOAA
  tab=load('../tmp/S_noaa_m09_ice.mtl');
  SouthAreaNoaa=tab(:,4) ;
  ...
  plot(year,vn,style) ; grid on ; axis tight ; title('Volume Arctic March')

  % AREA
  subplot(2,3,2) ; hold on
  plot(year,an,style) ; grid on ;  title('Area Arctic March')
  if year(1) > 1900 ; plot(datnoaa,NorthAreaNoaa,'b.-') ; axis tight ; end

  % EXTENT
  subplot(2,3,3) ; hold on
  plot(year,en,style) ; grid on ;  title('Extent Arctic March')
  ...
  plot(year,vs,style) ; grid on ; axis tight ; title('Volume Antarctic September')

  % AREA
  subplot(2,3,5) ; hold on
  plot(year,as,style) ; grid on ;  title('Area Antarctic September')
  xlabel(name,'fontsize',16)
  if year(1) > 1900 ; plot(datnoaa,SouthAreaNoaa,'b.-') ; axis tight ; end

  % EXTENT
  subplot(2,3,6) ; hold on
  plot(year,es,style) ; grid on ;  title('Extent Antarctic September')
...
%                eval(cmd)
             if maxyear ~= 0
                cmd=sprintf('%s  %s %s%s%s%s ','print','-djpeg100 ',plotdir,'/beg_icetrd.',figname,'.jpg') ;
             else
                cmd=sprintf('%s  %s %s%s%s%s ','print','-djpeg100 ',plotdir,'/icetrd.',figname,'.jpg') ;
             end
                %orient landscape;eval(cmd)
                eval(cmd)
                end
